...
def main(arg):
    ...
    file = datetime.now().strftime(arg['path']+database+'_(%Y-%m-%d_%H-%M-%S).sql')
...
arg.load(['database', 'source', 'path'], main, lambda x:
    print('python mysql_db_backup.py --source=username:password@host:port --database=db_name --path=path/to/dir'))